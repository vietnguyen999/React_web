...
const Add = ({ token }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${backenUrl}/api/product/add`, formData, {
        headers: { token },
      });
  ...
  return (
    <form onSubmit={onSubmitHandler} className="flex flex-col w-full items-start gap-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full sm:gap-8">
        ...
        <div>
          <p className="mb-2">Thể loại phụ</p>
          <select onChange={(e) => setSubCategory(e.target.value)} className="w-full px-3 py-2">
            <option value="Topwear">Áo Sơ Mi</option>
            <option value="Bottomwear">Quần Tây</option>
            <option value="Winterwear">Áo Thun</option>
          </select>
        </div>
